...
nur.ord2021 = ordinate(nuralgi2021, "NMDS","bray")
nur.ord2021
jwa2021=plot_ordination(nuralgi2021, nur.ord2021, color="pink_health", shape="location")+
  geom_point(size=6)+
...
#### PERMANOVA - WITH ALGICOLA 2021 ######
## run permanova 
perm.all= adonis2(all.mo2021[,-c(1:metacols)] ~ as.character(pink_health), 
                  data=all.mo2021, method = "bray")
perm.all


# betadispersion test
nur2021=subset_samples(nur, year_sampled==2021)
bdt <- phyloseq::distance(nur2021, method = "bray")
sample_df <- data.frame(sample_data(nur2021))
...
nur.ord2021 = ordinate(na2021, "NMDS","bray")
nur.ord2021
jna2021=plot_ordination(na2021, nur.ord2021, color="pink_health", shape="location")+
  geom_point(size=6)+
  scale_color_manual(values=c( "magenta", "#be9125"))+
  labs(color="substrate", shape="year")
...
#### PERMANOVA - NO ALGICOLA 2021 ######
# get data frames
otu2021 = as.data.frame(as.matrix(na2021@otu_table))
meta2021 = as.data.frame(as.matrix(na2021@sam_data))
## merge the data
all.mo = merge(meta2021, otu2021, by=0)
## count colum numbers
metacols = ncol(meta2021)+1
...
# betadispersion test
bdt <- phyloseq::distance(na2021, method = "bray")
sample_df <- data.frame(sample_data(na2021))
